...
log("配置文件位置:BDS根目录/plugins/SCFY/BDS/config.json")
log("插件版本:V1.1.4")
...
//跨服传送------------------------------------------------------------------------------------------------------------------------------------------------------------------
let servers = new JsonConfigFile("./plugins/SCFY/BDS/servers.json", JSON.stringify({
    server1name: "示例服务器",
...
//玩家信息模块
let playerlist = new JsonConfigFile("./plugins/SCFY/BDS/player.json")
mc.listen("onJoin", function (pl) {
    if (playerlist.get(pl.realName) == "true") { }
    else {
        log("玩家" + pl.realName + "的信息参数错误,已删除重构")
        var xuid = pl.xuid
        var uuid = pl.uuid
        playerlist.delete(pl.realName, "true")
        playerlist.delete(pl.realName + "uuid", uuid)
        playerlist.delete(pl.realName + "xuid", xuid)
